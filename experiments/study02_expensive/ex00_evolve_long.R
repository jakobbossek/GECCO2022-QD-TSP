...
ids = findNotDone()
ids = findExperiments(algo.pars = (target == "tour.length"))

submitJobs(ids = ids, resources = list(walltime = 60 * 60 * 10, mem = 4000))

ids = findExperiments(algo.pars = (target == "runtime"))
...
saveRDS(res, file = file.path(DATA.DIR, "study01_data.rds"))
jt = unwrap(getJobTable()[, c("job.id", "repl", "algo.pars")])
saveRDS(jt, file = file.path(DATA.DIR, "study01_jt.rds"))


res = reduceResultsList(findDone(ids))
saveRDS(res, file = file.path(DATA.DIR, "study01_expensive_data.rds"))
